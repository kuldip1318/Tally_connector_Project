...
export async function POST(req: Request) {
  try {
    ...
    const result = await tallySync.syncLedgers();
    ...
    console.error('Error during Tally data synchronization:', error);

    // Handle errors
    if (error instanceof Error) {
      return NextResponse.json(
        { message: 'Tally data synchronization failed', error: error.message },
        { status: 500 }
      );
    } else {
      return NextResponse.json(
        { message: 'Tally data synchronization failed', error: 'An unknown error occurred' },
        { status: 500 }
      );
    }
  }
